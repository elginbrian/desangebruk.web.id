"use client";

import { FiHome, FiFileText, FiBell, FiLogOut } from "react-icons/fi";
import { useState } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";

const Sidebar = () => {
  const pathname = usePathname();

  const menuItems = [
    { icon: FiHome, text: "Dashboard", href: "/dashboard" },
    { icon: FiFileText, text: "Kelola Berita", href: "/dashboard/article" },
    { icon: FiBell, text: "Kelola Pengumuman", href: "/dashboard/announcement" },
  ];

  return (
    <aside className="w-64 min-h-screen flex flex-col text-white bg-[#1B3A6D] shadow-lg">
      {/* Header */}
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0">
            <img
              src="/logo.png"
              alt="Desa Ngebruk Logo"
              className="w-10 h-10 object-contain"
              onError={(e) => {
                const target = e.currentTarget as HTMLImageElement;
                const svgElement = target.nextElementSibling as HTMLElement;
                target.style.display = "none";
                if (svgElement) {
                  svgElement.style.display = "block";
                }
              }}
            />
            <svg className="w-10 h-10 text-white hidden" fill="currentColor" viewBox="0 0 24 24">
              <path d="M10.554 6.24L4.43 2.184c-.18-.12-.43-.12-.61 0L3.82 2.184C2.9 2.184 2 3.084 2 4.004v15.992c0 .92.9 1.82 1.82 1.82h16.36c.92 0 1.82-.9 1.82-1.82V4.004c0-.92-.9-1.82-1.82-1.82L20.18 2.184c-.18-.12-.43-.12-.61 0l-6.124 4.056c-.18.12-.43.12-.61 0z" />
            </svg>
          </div>
          <div className="flex flex-col">
            <h1 className="text-sm font-bold text-white">Desa Ngebruk</h1>
            <p className="text-xs text-white/70">Panel Admin</p>
          </div>
        </div>
      </div>

      {/* Navigation */}
      <nav className="flex-grow px-4 py-4">
        <ul className="space-y-1">
          {menuItems.map((item, index) => (
            <li key={index}>
              <Link
                href={item.href}
                className={`flex items-center px-4 py-3 rounded-lg transition-all duration-200 text-xs ${pathname === item.href ? "bg-white text-[#1B3A6D] font-medium shadow-sm" : "text-white/90 hover:bg-white/10 hover:text-white"}`}
              >
                <item.icon className="mr-2" size={14} />
                <span>{item.text}</span>
              </Link>
            </li>
          ))}
        </ul>
      </nav>

      {/* Logout */}
      <div className="px-4 py-2 border-t border-white/10">
        <a href="#" className="flex items-center px-4 py-3 rounded-lg hover:bg-white/10 transition-all duration-200 text-xs text-white/90 hover:text-white">
          <FiLogOut className="mr-2" size={14} />
          <span>Logout</span>
        </a>
      </div>
    </aside>
  );
};

export default Sidebar;
